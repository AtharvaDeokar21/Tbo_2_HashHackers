\restrict Ie2Vuz3laHkK1I49vn4B6eKPxUFbwKYMLTNhbf7t0erYH6VdIpncQGw8siivnrJ
...
CREATE TABLE IF NOT EXISTS public.campaign_assets (
    ...
    created_at timestamp without time zone DEFAULT now(),
    CONSTRAINT campaign_assets_platform_check CHECK (((platform)::text = ANY ((ARRAY['instagram'::character varying, 'facebook'::character varying, 'google'::character varying, 'whatsapp'::character varying, 'email'::character varying])::text[])))
);
...
CREATE TABLE IF NOT EXISTS public.communication_logs (
    ...
    CONSTRAINT communication_logs_channel_check CHECK (((channel)::text = ANY ((ARRAY['whatsapp'::character varying, 'email'::character varying, 'call'::character varying])::text[])))
);
...
\unrestrict Ie2Vuz3laHkK1I49vn4B6eKPxUFbwKYMLTNhbf7t0erYH6VdIpncQGw8siivnrJ
